...
public final class FlatFileDatabaseManager implements DatabaseManager {
    ...
    FlatFileDatabaseManager(@NotNull final String usersFilePath, @NotNull final Logger logger, final long purgeTime,
        ...
        synchronized (fileWritingLock) {
            try {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    for (final int skill : skills.values()) {
                        if (skill != 0) {
                            powerless = false;
                            break;
                        }
    ...
    public void purgeOldUsers() {
        ...
        synchronized (fileWritingLock) {
            try {
                ...
                while ((line = in.readLine()) != null) {
                    final String[] character = line.split(":");
                    final String uuidString = character[UUID_INDEX];
                    ...
                    if (lastPlayed < 1 && (currentTime - lastPlayed > purgeTime)) {
                        ...
                        if (rewrite) {
                            ...
                            final String newLine = org.apache.commons.lang3.StringUtils.join(character,
                                    ":");
    ...
    public void writeUserToLine(@NotNull final PlayerProfile profile, @NotNull final Appendable appendable)
            throws IOException {
    ...
    private @NotNull UserQuery getUserQuery(@Nullable final UUID uuid, @Nullable final String playerName)
                            ...
                            if (!matchingName) {
                                logger.warning(
                                        "When loading user: " + playerName + " with UUID of ("
                                                ...
                                                + dbPlayerName + ")");
                                //logger.info("Name updated for player: " + rawSplitData[USERNAME_INDEX] + " => " + name);
    ...
    public @NotNull LeaderboardStatus updateLeaderboards() {
        ...
        final TreeSet<PlayerStat> mining = new TreeSet<>();
        final TreeSet<PlayerStat> woodcutting = new TreeSet<>();
        final TreeSet<PlayerStat> herbalism = new TreeSet<>();
        final TreeSet<PlayerStat> excavation = new TreeSet<>();
        final TreeSet<PlayerStat> acrobatics = new TreeSet<>();
        final TreeSet<PlayerStat> repair = new TreeSet<>();
        final TreeSet<PlayerStat> swords = new TreeSet<>();
        final TreeSet<PlayerStat> axes = new TreeSet<>();
        final TreeSet<PlayerStat> archery = new TreeSet<>();
        final TreeSet<PlayerStat> unarmed = new TreeSet<>();
        final TreeSet<PlayerStat> taming = new TreeSet<>();
        final TreeSet<PlayerStat> fishing = new TreeSet<>();
        final TreeSet<PlayerStat> alchemy = new TreeSet<>();
        final TreeSet<PlayerStat> crossbows = new TreeSet<>();
        final TreeSet<PlayerStat> tridents = new TreeSet<>();
        final TreeSet<PlayerStat> maces = new TreeSet<>();
    ...
    public @Nullable List<FlatFileDataFlag> checkFileHealthAndStructure() {
        ...
        final FlatFileDataProcessor dataProcessor;
    ...
    private void closeResources(final BufferedReader bufferedReader, final FileWriter fileWriter) {
    ...
    private int putStat(final TreeSet<PlayerStat> statList, final String playerName, final int value) {
        statList.add(new PlayerStat(playerName, value));
        return value;
    }

    private PlayerProfile loadFromLine(@NotNull final String[] character) {
        final Map<PrimarySkillType, Integer> skills = getSkillMapFromLine(character);      // Skill levels
    ...
    private void tryLoadSkillFloatValuesFromRawData(@NotNull final Map<PrimarySkillType, Float> skillMap,
        ...
        try {
            ...
            logger.severe(
                    "Data corruption when trying to load the statVal for skill " + primarySkillType
                            + " for player named " + userName + " setting statVal to zero");
            e.printStackTrace();
        }
    }
    ...
    private @NotNull Map<PrimarySkillType, Integer> getSkillMapFromLine(
        ...
        final EnumMap<PrimarySkillType, Integer> skills = new EnumMap<>(
